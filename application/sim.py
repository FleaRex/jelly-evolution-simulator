...
class Sim:
    ...
    def __init__(self, creature_count: int, config: dict) -> None:
        ...
        self.S_VISIBLE: float = (
            0.05  # what proportion of the population does a species need to appear on the SAC graph?
        )
        self.S_NOTABLE: float = (
            0.10  # what proportion of the population does a species need to appear in the genealogy?
        )
        self.HUNDRED: int = (
    ...
    def simulate_import(self, gen, start_index, end_index, from_calm_state):
        node_coor = self.get_starting_node_coor(
            gen, start_index, end_index, from_calm_state
        )
        ...
        for f in range(frame_count):
            ...
            if not calming_run:  # dealing with collision with the ground.
                nodes_touching_ground = np.ma.masked_where(
                    node_coor[:, :, :, 1] >= floor_y, node_coor[:, :, :, 1]
                )
                m = nodes_touching_ground.mask.astype(
                    float
                ...
                node_coor[:, :, :, 1] = np.clip(
                    node_coor[:, :, :, 1], ceiling_y, floor_y
                )  # clip nodes below the ground back to ground level
                node_coor[
                    :, :, :, 2
                ] *= ground_friction_multiplier  # any nodes touching the ground must be slowed down by ground friction.

        if (
            calming_run
        ):  # If it's a calming run, then take the average location of all nodes to center it at the origin.
            node_coor[:, :, :, 0] -= np.mean(
                node_coor[:, :, :, 0], axis=(1, 2), keepdims=True
            )
    ...
    def do_species_info(self, nsp, best_of_each_species) -> None:
        ...
        for sp in nsp.keys():
            ...
            if (
                pop >= self.creature_count * self.S_NOTABLE and not info.prominent
            ):  # prominent threshold
