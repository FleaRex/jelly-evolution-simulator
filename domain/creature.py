...
class Creature:
    ...
    def draw_icon(self, icon_dim, bg_color, beat_fade_time: int) -> Surface:
        ...
        transform = [
            icon_dim[0] / 2,
            icon_dim[0] / (self.sim.width + 2),
            icon_dim[0] / (self.sim.height + 2.85),
        ]
        self.draw_creature(
            icon, self.calmState, beat_fade_time, transform, False, False
        )
        r = icon_dim[0] * 0.09
        r2 = icon_dim[0] * 0.12
        pygame.draw.circle(
            icon, species_to_color(self.species, self.ui), (icon_dim[0] - r2, r2), r
        )
    ...
    def traits_to_color(self, dna, x, y, frame):
        ...
        location_index = x * self.sim.height + y
        dna_index = (
            location_index * self.sim.beats_per_cycle + beat
        ) * self.sim.traits_per_box
        dna_index_prev = (
            location_index * self.sim.beats_per_cycle + beat_prev
        ) * self.sim.traits_per_box
        ...
        if self.codon_with_change is not None:
            ...
            if (
                ...
                < dna_index + self.sim.traits_per_box
            ):
                next_green = 1
            ...
            if (
                dna_index_prev
                <= self.codon_with_change
                < dna_index_prev + self.sim.traits_per_box
            ):
                prev_green = 1
