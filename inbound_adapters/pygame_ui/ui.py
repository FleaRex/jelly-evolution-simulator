...
class UI:
    ...
    def __init__(self, config: dict):
        ...
        self.big_font = pygame.font.Font(font, 60)
        self.small_font = pygame.font.Font(font, 30)
        self.tiny_font = pygame.font.Font(font, 21)
        ...
        self.mosaic_screen_height = (
            self.window_height - self.menu_text_up - self.cm_margin_1 * 2
        )

        s1: int = int(
            self.mosaic_screen_width_creatures / self.mosaic_dim[0]
            - self.cm_margin_2 * 2
        )
        s2: int = int(
            self.mosaic_screen_width_creatures / self.mosaic_dim[1]
            - self.cm_margin_2 * 2
        )
        ...
        self.mosaic_screen: Surface = pygame.Surface(
            (self.mosaic_screen_width_creatures, self.mosaic_screen_height),
            pygame.SRCALPHA,
            32,
        )
        self.info_bar_screen: Surface = pygame.Surface(
            (self.info_w, self.mosaic_screen_height), pygame.SRCALPHA, 32
        )
        self.preview_locations: list[list[int]] = [
            [570, 105, 250, 250],
            [570, 365, 250, 250],
            [570, 625, 250, 250],
        ]
    ...
    def add_buttons_and_sliders(self):
        self.gen_slider: Slider = Slider(
            pdim=(40, self.window_height - 100, self.window_width - 80, 60, 140),
            callback_func=self.update_gen_slider,
        )
        ...
        self.show_creatures_button = Button(
            button_coor[0], ["Show creatures", "Hide creatures"], self.toggle_creatures
        )
        ...
        self.sample_button = Button(
            button_coor[3], ["Watch sample", "Stop sample"], self.start_sample
        )
        ...
        self.alap_button = Button(
            button_coor[5], ["Turn on ALAP", "Turn off ALAP"], self.do_nothing
        )
    ...
    def detect_mouse_motion(self):
        ...
        if self.mosaic_visible:
            ...
            if (
                0 <= rel_mouse_x < self.mosaic_screen_width_creatures
                and 0 <= rel_mouse_y < self.mosaic_screen_height
            ):
            ...
            for r in range(len(self.preview_locations)):
                ...
                if (
                    pl[0] <= mouse_x < pl[0] + pl[2]
                    and pl[1] <= mouse_y < pl[1] + pl[3]
                ):
        ...
        if new_clh[1] != self.creature_location_highlight[1]:
            ...
            if self.creature_location_highlight[1] is None:
                ...
                self.creature_highlight = [
                    self.sim.creatures[gen][self.creature_location_highlight[1]]
                ]
                self.visual_sim_memory = [
                    self.sim.simulate_import(
                        gen,
                        self.creature_location_highlight[1],
                        self.creature_location_highlight[1] + 1,
                        True,
                    )
                ]
                self.movie_screens = [None] * 1
                self.draw_info_bar_creature(
                    self.sim.creatures[gen][self.creature_location_highlight[1]]
                )
    ...
    def draw_movie_quad(self, species) -> None:
        ...
        self.draw_movie_grid(
            self.screen, coor, mask, titles, [Color.GRAYISH] * l, self.tiny_font
        )
    ...
    def draw_info_bar_species(self, species) -> None:
        ...
        self.draw_lightboard(
            ...
            (
                ...
                self.info_w * 0.37,
                self.mosaic_screen_height - self.movie_single_dim[1] - 20,
            ),
        )
    ...
    def draw_menu_text(self) -> None:
        ...
        gen_surface = self.big_font.render(
        ...
        for i in range(l):
            ...
            try:
                dim = array_int_multiply(
                    self.movie_single_dim, mscale[self.creature_location_highlight[0]]
                )
    ...
    def get_highlighted_species(self):
        ...
        if self.creature_location_highlight[0] == 2:
            ...
            self.creature_location_highlight[0] == 0
            or self.creature_location_highlight[0] == 1
        ):
    ...
    def display_creature_mosaic(self, screen):
        time_since_last_press = (
            time.time() - self.show_creatures_button.time_of_last_click
        )
        pan_time = 0.2
        ...
        if not self.mosaic_visible and frac < 1:
            self.screen.blit(
                ...
                (
                    self.cm_margin_1,
                    self.cm_margin_1 - self.mosaic_screen.get_height() * frac,
                ),
            )
